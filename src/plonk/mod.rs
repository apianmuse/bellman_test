...
pub fn verify<E: Engine, T: Transcript<E::Fr>>(
    ...
    println!("== in bellman mod.rs verify ==");
    self::better_cs::verifier::verify::<E, PlonkCsWidth4WithNextStepParams, T>(&proof, &verification_key, None)
}

//新增:聚合
pub fn aggregate_verify<E: Engine, P: PlonkConstraintSystemParams<E> ,T: Transcript<E::Fr>>(
    proof: &Proof<E, P>,
    verification_key: &VerificationKey<E, P>
) -> Result<bool, SynthesisError> {
    println!("== in bellman mod.rs aggregate_verify ==");
    use std::fs::File;
    ...
    //读文件
    let mut reader = std::io::BufReader::with_capacity(1 << 24, File::open("E:/plonk/plonkit0513/test_agg/circuits/aggragate/vk.bin").expect("read vk file err"));
    let my_vk = VerificationKey::<E,P>::read(&mut reader).expect("read vk err");
    //放入数组
    let mut generator = vec![];
    let mut x = vec![];
    for i in 1..=30{
        let proof_path = "E:/plonk/plonkit0513/test_agg/circuits/aggragate/proof/proof".to_string() + &i.to_string()[..] + ".bin";
        let my_proof = Proof::<E, P>::read(File::open(proof_path).expect("read proof file err")).expect("read proof err");
        let (valid, [pair_with_generator,pair_with_x]) = self::better_cs::verifier::verify_and_aggregate::<E, P, T>(&my_proof, &my_vk, None)?;
