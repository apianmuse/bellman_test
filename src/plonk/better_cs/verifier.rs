...
pub fn verify<E: Engine, P: PlonkConstraintSystemParams<E>, T: Transcript<E::Fr>>(
    ...
    // use std::fs::File;
    // println!("== in bellman verifier.rs verify ==");
    // //读文件
    // let mut reader = std::io::BufReader::with_capacity(1 << 24, File::open("E:/plonk/plonkit0513/mytest/zk/vk.bin").expect("read vk file err"));
    // let my_vk = VerificationKey::<E,P>::read(&mut reader).expect("read vk err");
    // let my_proof = Proof::<E, P>::read(File::open("E:/plonk/plonkit0513/mytest/zk/proof.bin").expect("read proof file err")).expect("read proof err");
    // let (valid, [pair_with_generator,pair_with_x]) = verify_and_aggregate::<E, P, T>(&my_proof, &my_vk, transcript_init_params.clone())?;
    // //println!("valid = {:#?}",valid);
    // println!("pair_with_generator = {:#?}",pair_with_generator);
    // println!("pair_with_x = {:#?}",pair_with_x);

    // let mut reader2 = std::io::BufReader::with_capacity(1 << 24, File::open("E:/plonk/plonkit0513/mytest/zk2/vk.bin").expect("read vk file err"));
    // let my_vk2 = VerificationKey::<E,P>::read(&mut reader2).expect("read vk err");
    // let my_proof2 = Proof::<E, P>::read(File::open("E:/plonk/plonkit0513/mytest/zk2/proof.bin").expect("read proof file err")).expect("read proof err");
    // let (valid2, [pair_with_generator2,pair_with_x2]) = verify_and_aggregate::<E, P, T>(&my_proof2, &my_vk2, transcript_init_params.clone())?;
    // //println!("valid2 = {:#?}",valid2);
    // println!("pair_with_generator2 = {:#?}",pair_with_generator2);
    // println!("pair_with_x2 = {:#?}",pair_with_x2);

    // let mut reader3 = std::io::BufReader::with_capacity(1 << 24, File::open("E:/plonk/plonkit0513/mytest/zk3/vk.bin").expect("read vk file err"));
    // let my_vk3 = VerificationKey::<E,P>::read(&mut reader3).expect("read vk err");
    // let my_proof3 = Proof::<E, P>::read(File::open("E:/plonk/plonkit0513/mytest/zk3/proof.bin").expect("read proof file err")).expect("read proof err");
    // let (valid3, [pair_with_generator3,pair_with_x3]) = verify_and_aggregate::<E, P, T>(&my_proof3, &my_vk3, transcript_init_params.clone())?;
    // //println!("valid3 = {:#?}",valid3);
    // println!("pair_with_generator3 = {:#?}",pair_with_generator3);
    // println!("pair_with_x3 = {:#?}",pair_with_x3);

    // // let mut reader4 = std::io::BufReader::with_capacity(1 << 24, File::open("E:/plonk/plonkit0513/mytest/zk4/vk.bin").expect("read vk file err"));
    // // let my_vk4 = VerificationKey::<E,P>::read(&mut reader4).expect("read vk err");
    // // let my_proof4 = Proof::<E, P>::read(File::open("E:/plonk/plonkit0513/mytest/zk4/proof.bin").expect("read proof file err")).expect("read proof err");
    // // let (valid4, [pair_with_generator4,pair_with_x4]) = verify_and_aggregate::<E, P, T>(&my_proof4, &my_vk4, transcript_init_params.clone())?;
    // // //println!("valid4 = {:#?}",valid4);
    // // println!("pair_with_generator4 = {:#?}",pair_with_generator4);
    // // println!("pair_with_x4 = {:#?}",pair_with_x4);

    // //聚合

    // //随机数,Fr
    // let c = E::Fr::from_str("12822881022193341620569536695323107057941081797098984275025377625162521948606").unwrap();
    // //0x1c597e53920df5dc532be5a53e25e1b6d3e4c3511f1352e7ca8180a60d02b1be
    // println!("c = {:#?}",c); 
    // let mut scalars = vec![]; //c^i
    // let mut current = c.clone();
    // scalars.push(current.clone());
    // for i in 1..3 {
    //     current.mul_assign(&c);
    //     scalars.push(current.clone());
    // }
    ...
    // generator.push(pair_with_generator);
    // generator.push(pair_with_generator2);
    // generator.push(pair_with_generator3);
    ...
    // let mut generator_c = vec![];
    // let mut x_c = vec![];
    // for i in 0..3 {
    //     generator_c.push(generator[i].mul(scalars[i].into_repr()));
    //     x_c.push(x[i].mul(scalars[i].into_repr()));
    // }
    // //add
    // let mut aggregate_generator = generator_c[0];
    // let mut aggregate_x = x_c[0];
    // for i in 1..3 {
    //     aggregate_generator.add_assign(&generator_c[i]);
    //     aggregate_x.add_assign(&x_c[i]);
    // }
    // // G1: CurveProjective 需转为 G1Affine: CurveAffine   
    // let mut aggregate_generator_affine = aggregate_generator.into_affine();
    // let mut aggregate_x_affine = aggregate_x.into_affine();
    // println!("aggregate_generator_affine = {:#?}",aggregate_generator_affine); 
    // println!("aggregate_x_affine = {:#?}",aggregate_x_affine); 
...
//     fn rand_test(){
//         //随机数c
//         //let c = BlsScalar::random(&mut rand::thread_rng());
//         let c = BlsScalar([
//             0x47c0649f712700ea,
//             0x1554b1bf8f109fa8,
//             0x5d042b18543c6478,
...
//         ]);
//         println!("{:?}",c);
//         //47c0649f712700ea1554b1bf8f109fa85d042b18543c647878578c62e8845406 //32字节
//         //计算c^i
//         let powers = powers_of(&c,3);
//         println!("{:#?}",powers);   
